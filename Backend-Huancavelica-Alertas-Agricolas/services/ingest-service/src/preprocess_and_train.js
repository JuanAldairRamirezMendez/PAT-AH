...
(async ()=>{
  ...
  // Convert field5 to number and detect missing sentinel -99.9
  ...
  // Build new dataset with lags and rolling stats
  ...
  for (let i = 0; i < rows.length; i++){
    ...
    if (isNaN(target)) continue; // drop rows without target
    // drop if lag1 or lag7 or roll7 missing (we want full features)
    ...
    // only include rows where features are numeric
  ...
  // Normalize (z-score) using mean/std on whole preprocessed set
  ...
  const trainCmd = `curl.exe -v -X POST "http://localhost:4000/ai/train-model" -H "Content-Type: application/json" --data-binary @"${trainBodyPath}"`;
  console.log('Calling /ai/train-model...');
